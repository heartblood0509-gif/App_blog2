...
def normalize_nfc(text):
    """한글 자모 분해형(NFC 아님)을 완성형(NFC)으로 정규화.

    맥에서 붙여넣은 대본은 "두"가 "ㄷ+ㅜ"(분해형, NFD)로 올 수 있다. 눈에는 같아도
    글자 수가 2배로 세어져 화면 폭 판정이 오탐하고(화면엔 다 들어가는데 "화면보다 길어요"),
    FFmpeg drawtext 는 자모 합성을 안 해 최종 영상 자막이 깨져 그려질 수 있다.
    프론트 subtitle-split.ts 의 .normalize("NFC") 와 동일 규칙."""
...
def display_len(text):
    ...
    NFC 정규화 필수(맥 붙여넣기 대본의 자모 분해형 오탐 방지 — normalize_nfc 참고)."""
    return len(re.sub(r'[?,!.~…·\'""]', "", normalize_nfc(text)))
...
def _norm_joined(s):
    ...
    NFC 정규화 — 조각/word_times 인코딩형이 달라도 같은 글자면 정합하도록."""
    return re.sub(r"\s+", "", normalize_nfc(s)).rstrip(".")
