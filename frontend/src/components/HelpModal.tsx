    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Popup
          className={cn(
            "fixed left-1/2 top-1/2 z-50 flex h-[88vh] w-[min(1240px,95vw)] -translate-x-1/2 -translate-y-1/2",
            "flex-col overflow-hidden rounded-2xl bg-popover text-popover-foreground",
        ...
        >
          <header className="flex items-start justify-between gap-4 border-b border-foreground/5 px-10 py-7">
            <div className="min-w-0">
              <DialogPrimitive.Title className="font-heading text-[26px] font-semibold tracking-tight text-foreground">
                사용 매뉴얼
              </DialogPrimitive.Title>
              <DialogPrimitive.Description className="mt-2 text-[15px] text-muted-foreground">
                위에서부터 차근차근 따라하시면 첫 글 발행까지 가능합니다.
...
function HelpToc() {
  return (
    <nav
      aria-label="목차"
      className="hidden w-[310px] shrink-0 overflow-y-auto border-r border-foreground/5 bg-gradient-to-b from-muted/40 to-muted/15 px-5 py-9 md:block"
    >
      {/* 헤더 — 작은 primary dot + 라벨 */}
        ...
        {SECTIONS.map((s, i) => (
          <li key={s.id}>
            <a
              href={`#${s.id}`}
              className={cn(
                "group relative flex items-center gap-3.5 rounded-lg px-3 py-3.5 text-[15.5px] font-medium tracking-[-0.005em] text-foreground/70",
      ...
      {/* 푸터 — 미묘한 안내 */}
      <p className="mt-8 border-t border-foreground/5 px-2 pt-5 text-[12px] leading-relaxed text-muted-foreground/80">
        항목을 누르면 해당 위치로
        <br />
        바로 이동합니다.
      </p>
...
function HelpBody() {
  return (
    <div className="min-w-0 flex-1 overflow-y-auto">
      <article className="mx-auto max-w-[840px] px-14 py-14 text-[18px] leading-[1.95] tracking-[0.005em] text-foreground/90">
        {/* 시작 안내 */}
        ...
        <Section id="warning" title="주의사항" tone="warning">
          <p>
            이 도구는{" "}
            <strong>네이버의 자동 로그인·자동 발행</strong> 기능을 사용합니다.
          </p>
          <ul>
            <li>
            ...
            <li>
              <strong>자동 발행은 하루 최대 4편을 권장</strong>합니다 — 계정
              2개 × 계정당 2편이 안전선입니다. 글이 더 필요하시다면 블로그픽으로
              본문만 생성한 뒤{" "}
              <strong>직접 네이버 블로그에 복사·붙여넣어</strong> 발행해주세요.
              그 이상 자동 발행을 무리하게 돌리면 계정 차단 위험이 커서 권장하지
              않습니다.
            </li>
          </ul>
        </Section>
        ...
        <Section id="wizard" title="글쓰기 5단계 위저드">
          ...
          <ul>
            <li>
              <strong>이미지 첨부</strong>: 자동으로 들어간 이미지 외에 직접
              추가 가능
            </li>
            <li>
        ...
        <Section id="network" title="회사 컴퓨터 · 회사 망에서 쓰시나요?">
          <p>
            블로그픽은 인터넷으로 다음 도메인에 접속합니다. 회사 방화벽에 막힌다면{" "}
      ...
      >
        ...
        <h2
          className={cn(
            ...
            tone === "warning" ? "text-amber-900 dark:text-amber-100" : "text-foreground",
          )}
        >
          {title}
        </h2>
...
// 헤더에서 모달 상태 관리하기 쉽게 hook 형태로도 제공
export function useHelpModal() {
  const [open, setOpen] = useState(false);
  return {
    open,
    setOpen,
    helpModalProps: { open, onOpenChange: setOpen },
  };
}
