...
export function detectBrandLeakInProposition(
  ...
  if (profile.label && profile.label !== profile.name) candidates.push(profile.label);
  if (profile.narrator?.name) candidates.push(profile.narrator.name);
  if (profile.supportingPersona?.name) candidates.push(profile.supportingPersona.name);
  for (const phrase of profile.signaturePhrases || []) {
    if (phrase && phrase.length >= 4) candidates.push(phrase);
  }
...
- 화자: ${profile.narrator?.name || ""} (${profile.narrator?.role || ""}) — 권위: ${profile.narrator?.authority || ""}
- 보조 인물: ${profile.supportingPersona?.name || ""} (${profile.supportingPersona?.role || ""}) — 권위: ${profile.supportingPersona?.authority || ""}
...
- 권위·신뢰 자산:
${(profile.authorityAssets || []).map((a) => `  · ${a}`).join("\n") || "  (없음)"}
- 추가 서비스:
...
- 시그니처 표현:
${(profile.signaturePhrases || []).map((p) => `  · ${p}`).join("\n") || "  (없음)"}
- 추천 코스/상품:
...
1. 금기 (명제 텍스트에 절대 등장 X)
   - 브랜드명·라벨: "${profile.name}", "${profile.label}"
   - 인물 실명: "${profile.narrator?.name || ""}", "${profile.supportingPersona?.name || ""}"
   - 자사 서비스·상품 고유명사
   - 시그니처 표현 그대로 인용 (의미는 차용 OK, 문장 그대로는 X)
...
✗ "${(profile.signaturePhrases || ["캐리어 한 번만 풀면 된다"])[0]}" (시그니처 그대로 인용)`);
...
source 라벨은 어느 프로필 항목에서 우러났는지 짧게 (예: "차별점#2", "권위자산#3", "빌런·비유").
