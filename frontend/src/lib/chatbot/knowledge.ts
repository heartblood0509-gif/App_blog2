...
/** 답변 깊이 — 사용자가 "더 자세히/짧게"를 누르면 전달된다. 미지정 시 기본 톤. */
export type Verbosity = "detailed" | "concise";

/**
 * 시스템 프롬프트 빌더.
 * @param pathname 사용자가 현재 보고 있는 경로(선택) — 현재 화면 맥락 주입용.
 * @param verbosity 이번 답변의 깊이(선택) — "더 자세히/짧게" 버튼이 전달.
 */
export function buildSystemPrompt(
  ...
  // "더 자세히/짧게" 버튼이 누른 깊이. 이 지시는 아래 [지식 베이스] "뒤"에 배치돼
  // "마지막 지시"가 된다(최신성 가중) — 도입부·규칙의 "간결" 신호를 이기도록.
  const verbosityLine =
    verbosity === "detailed"
      ? `\n\n[이번 답변 지시 — 위 지식 베이스보다 우선]\n사용자가 "더 자세히"를 눌렀습니다. 이 답변에 한해 도입부의 "간결한"과 규칙 1·5의 "짧게/간결"보다 이 지시를 우선하세요(길어져도 좋습니다). 바로 위 사용자 메시지가 가리키는 "직전 답변"을 토대로, 사용자가 물은 주제에 한정해 [지식 베이스]의 직접 관련 내용을 매뉴얼 수준으로 풀어쓰세요. 단계형 주제(설치·발급·발행 등)는 번호를 매긴 단계로, 개념형 주제(무엇/왜)는 짧은 소제목·불릿으로 정리하세요. 관련되면 전제 조건·정확한 화면 위치·버튼 이름·각 단계·자주 겪는 문제와 해결을 포함하되, 해당 없는 항목은 억지로 넣지 마세요. 결제·요금·429·한도 같은 인접 주제는 사용자가 그 단어를 언급했을 때만 포함하세요. 지식 베이스에 해당 내용이 적으면 억지로 늘리지 말고 아는 사실에서 멈추며, 없는 내용은 지어내지 마세요.`
      : verbosity === "concise"
...
4. ${contactLine}
5. 도움말 페이지로 떠넘기지 말고, [지식 베이스] 내용을 바탕으로 그 자리에서 완결된 답을 제공하세요. 답변 끝에 "도움말에서 자세히 보기" 같은 링크를 덧붙이지 마세요. 화면 위치는 문장으로 안내하세요(예: "설정 > API 키 설정 메뉴"). 지식 베이스에 정보가 없으면 지어내지 말고 위 4번의 문의처로 안내하세요. 기본은 핵심만 간결하게 답하고, 더 깊은 설명이 필요하면 사용자가 "더 자세히" 버튼을 누릅니다.
6. 첨부 이미지(에러·화면 캡처)로 원인을 추정할 때는 단정하지 말고 "~로 보입니다"처럼 표현하고, 글씨가 흐리거나 불확실하면 그 사실을 밝힌 뒤 확인 절차나 문의처를 안내하세요.${pageContext}

[지식 베이스]
${HELP_MANUAL_TEXT}${verbosityLine}`;
}
