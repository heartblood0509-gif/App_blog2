...
export async function POST(request: Request) {
  ...
  const oldDeviceId = source.old_device_id;
  const device = parseDeviceInfo(source.device);
  if (typeof oldDeviceId !== "string" || !oldDeviceId || !device) {
  ...
  const { data, error } = await authorized.supabase.rpc("replace_device", {
    p_old_device_id: oldDeviceId,
    p_new_device_id: device.device_id,
